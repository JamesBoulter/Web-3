...
function processingFeeCents(amountCents, platformFeeAmountCents) {
  const percent = Number(process.env.STRIPE_PROCESSING_PERCENT || 2.9) / 100;
  const fixed = Number(process.env.STRIPE_PROCESSING_FIXED_CENTS || 30);
  const subtotal = Number(amountCents) + Number(platformFeeAmountCents);

  if (!Number.isFinite(percent) || percent < 0 || percent >= 1) return 0;
  if (!Number.isFinite(fixed) || fixed < 0) return 0;

  const totalNeeded = Math.ceil((subtotal + fixed) / (1 - percent));
  return Math.max(0, totalNeeded - subtotal);
}
