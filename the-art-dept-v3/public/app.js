...
async function uploadFile(file) {
  return uploadPublicFile(file, 'portfolio');
}
...
async function createWatermarkedPreview(file) {
  ...
  for (let y = -height; y <= height; y += stepY) {
    for (let x = -width; x <= width; x += stepX) {
      ctx.strokeText('THE ART DEPT', x, y);
      ctx.fillText('THE ART DEPT', x, y);
    }
  }
  ctx.restore();
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText('The Art Dept preview - clean file unlocks after purchase', 18, height - Math.max(22, Math.round(height * 0.035)));
