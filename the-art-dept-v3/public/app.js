...
function renderOrdersTable(orders, title) {
  return `
    <section class="table-wrap">
      ...
      ${orders.length ? '<table><thead><tr><th>Status</th><th>Artist</th><th>Amount</th><th>Platform fee</th><th>Date</th></tr></thead><tbody>' +
        orders.map((order) => '<tr><td><span class="status-pill">' + escapeHtml(order.status) + '</span></td><td>' + escapeHtml(artistName(order.artist_id)) + '</td><td>' + formatMoney(order.amount_cents) + '</td><td>' + formatMoney(order.platform_fee_cents) + '</td><td>' + new Date(order.created_at).toLocaleDateString() + '</td></tr>').join('') +
...
function updateAuthMode() {
  ...
  qsa('.signup-only').forEach((item) => item.hidden = !isSignup);
}
...
async function handleAuthSubmit(event) {
  ...
  if (state.authMode === 'signup') {
    ...
    if (error) throw error;
    if (!data.session) showToast('Account created. Check your email if Supabase asks you to confirm it.');
  } else {
...
async function init() {
  ...
  if (params.get('payment') === 'success') showToast('Payment complete.');
