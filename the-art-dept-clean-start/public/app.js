    ...
    '<article class="product-card">' +
      ...
      '<div class="product-card-body">' +
        ...
        '<div class="card-actions">' +
          (product.canBuy
            ? '<button class="primary" type="button" data-listing-buy="' + escapeHtml(product.id) + '">' + icon('i-cart') + 'Buy now</button>'
            : '<button class="secondary" type="button" disabled>' + icon('i-wallet') + 'Payout needed</button>') +
          '<button class="secondary" type="button" data-listing-request="' + escapeHtml(product.id) + '">' + icon('i-brush') + 'Request similar</button>' +
        '</div>' +
      '</div>' +
    '</article>'
  )).join('');
...
async function publishListing(event) {
  ...
  payload.connectedAccountId = getSavedConnectedAccountId();

  if (!payload.connectedAccountId) {
    qs('#artist-signup').scrollIntoView({ behavior: 'smooth', block: 'start' });
    showToast('Connect Stripe payouts before publishing a listing.');
    return;
  }
...
async function buyListing(listingId) {
  ...
  try {
    ...
    const data = await callFunction('/.netlify/functions/create-listing-checkout', {
      listingId
    });
    window.location.href = data.url;
  } catch (error) {
    showToast(error.message || 'This listing is not ready for checkout yet.');
  }
}
...
function bindEvents() {
  ...
  qs('#productGrid').addEventListener('click', (event) => {
    ...
    const buy = event.target.closest('[data-listing-buy]');
    ...
    if (buy) buyListing(buy.dataset.listingBuy);
