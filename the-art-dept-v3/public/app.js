...
function renderOrderRow(order) {
  ...
  const deliverable = canDownload
    ...
    : commissionRequest
      ? commissionRequest.final_file_path && ['paid', 'fulfilled'].includes(order.status)
        ? '<button class="secondary compact" data-download-commission="' + commissionRequest.id + '">' + icon('i-image') + 'Download clean final</button>'
...
function renderRequestDetail(request) {
  ...
  const artistOptions = artistModels().map((item) => '<option value="' + item.profile.id + '" ' + (request.artist_id === item.profile.id ? 'selected' : '') + '>' + escapeHtml(item.profile.display_name) + '</option>').join('');
  const artistField = isAdmin
    ? '<label>Assigned artist<select name="artist_id"><option value="">Choose artist</option>' + artistOptions + '</select></label>'
    : '<input type="hidden" name="artist_id" value="' + escapeHtml(request.artist_id || '') + '" />';
  const safetyNote = canManage
    ? 'Accept the request, request payment through Stripe, then upload drafts after the payment says paid.'
    : paid
      ? 'Payment is on record. Drafts and the clean final file will appear here when the artist uploads them.'
      : request.quoted_cents
        ? 'Review the quote and pay through Stripe when you are ready to start the commission.'
        : 'Your request has been received. A quote will appear here after an artist reviews it.';
  const draftSubtitle = canManage
    ? (paid ? 'Upload watermarked drafts here for customer review.' : 'Draft upload unlocks after the customer pays through Stripe.')
    : (paid ? 'Watermarked drafts from the artist will appear here for review.' : 'Drafts will appear here after the commission is paid and the artist uploads them.');
  const finalSubtitle = canManage
    ? (paid ? 'Upload the clean final file here after the customer approves the work.' : 'Final clean delivery unlocks after the customer pays through Stripe.')
    : request.final_file_path
      ? 'Your clean final file is ready to download.'
      : (paid ? 'The artist will upload the clean final here when the commission is ready.' : 'The clean final file will be available here after payment and delivery.');
  const draftEmptyText = canManage
    ? (paid ? 'Upload a watermarked draft for customer review.' : 'Payment should be collected before drafts are shared.')
    : (paid ? 'No draft has been uploaded yet.' : 'No draft is available yet.');
  const finalEmptyText = canManage
    ? (paid ? 'Upload the clean final when the commission is ready to deliver.' : 'Payment should be collected before clean files are uploaded.')
    : (paid ? 'No clean final has been uploaded yet.' : 'No clean final is available yet.');
    ...
    <div class="request-detail">
      ...
      <div class="request-detail-grid">
        ...
        <section class="detail-panel">
          ...
          <p class="form-note">${escapeHtml(safetyNote)}</p>
      ...
      ${canManage ? `
        <section class="detail-panel">
          ...
          <form class="form-stack compact-form" data-form="commission-quote">
            ...
            ${request.artist_id ? '' : '<p class="form-note">Assign an artist before saving the quote so the customer can pay the right person through Stripe.</p>'}
      ...
      <section class="detail-panel">
        <div class="split-head">
          <div><h3>Draft delivery</h3><p class="muted">${escapeHtml(draftSubtitle)}</p></div>
        </div>
        ${request.draft_url ? `
          <div class="draft-preview">
            ...
            <div>
              ...
              <button class="secondary compact" data-download-image="${escapeHtml(request.draft_url)}" data-download-title="${escapeHtml(request.title + ' watermarked draft')}">${icon('i-image')}Download watermarked draft</button>
            </div>
          </div>
        ` : renderEmpty('No draft uploaded yet', draftEmptyText)}
        ${canManage ? `
      ...
      <section class="detail-panel">
        <div class="split-head">
          <div><h3>Final delivery</h3><p class="muted">${escapeHtml(finalSubtitle)}</p></div>
          ${request.final_file_path ? '<span class="status-pill">final uploaded</span>' : '<span class="status-pill">waiting</span>'}
        </div>
        ${request.final_file_path ? `
          <div class="delivery-box">
            <div>
              <strong>Clean final file is ready</strong>
              <p>${escapeHtml(request.final_note || 'No note added.')}</p>
              <p class="muted">Only the paid customer, the artist, and admins can download this file.</p>
            </div>
            <button class="primary compact" data-download-commission="${escapeHtml(request.id)}">${icon('i-image')}Download clean final</button>
          </div>
        ` : renderEmpty('No final file uploaded yet', finalEmptyText)}
        ${canManage ? `
          <form class="form-stack compact-form" data-form="commission-final">
            <input type="hidden" name="request_id" value="${escapeHtml(request.id)}" />
            <label>Clean final image<input name="file" type="file" accept="image/*" ${paid ? '' : 'disabled'} required /></label>
            <label>Delivery note<textarea name="final_note" rows="3" ${paid ? '' : 'disabled'} placeholder="Add a note for the customer.">${escapeHtml(request.final_note || '')}</textarea></label>
            <button class="primary" type="submit" ${paid ? '' : 'disabled'}>${icon('i-send')}Upload clean final</button>
          </form>
        ` : ''}
      </section>
    </div>
...
async function uploadCommissionFinalFile(file) {
  const path = state.profile.id + '/commission-finals/' + Date.now() + '-' + slug(file.name);
  const { error } = await state.supabase.storage.from('listing-originals').upload(path, file, {
    upsert: false,
    contentType: file.type || undefined
  });
  if (error) throw error;
...
async function handleSubmit(event) {
  ...
  if (type === 'commission-quote') {
    ...
    const assignedArtistId = data.artist_id || request.artist_id || (state.profile && state.profile.role === 'artist' ? state.profile.id : '');
    if (!assignedArtistId) {
      showToast('Assign an artist before sending the quote.');
      return;
    }
    if (quoteCents < 100) {
    ...
    if (data.status === 'completed' && (request.payment_status || 'not_requested') !== 'paid') {
    ...
    if (state.profile && assignedArtistId !== request.artist_id) {
      updates.assigned_by = state.profile.id;
      updates.assigned_at = new Date().toISOString();
      updates.declined_reason = null;
      updates.declined_at = null;
    }
    let { error } = await state.supabase.from('commission_requests').update(updates).eq('id', data.request_id);
    if (error && /assigned_by|assigned_at|declined_reason|declined_at|column/i.test(error.message || '')) {
      delete updates.assigned_by;
      delete updates.assigned_at;
      delete updates.declined_reason;
      delete updates.declined_at;
      const retry = await state.supabase.from('commission_requests').update(updates).eq('id', data.request_id);
      error = retry.error;
    }
    if (error) throw error;
  ...
  if (type === 'commission-draft') {
    ...
    const { error } = await state.supabase.from('commission_requests').update({
      draft_url: draftUrl,
  ...
  if (type === 'commission-final') {
    ...
    if ((request.payment_status || 'not_requested') !== 'paid') {
      showToast('Collect payment through Stripe before uploading the clean final.');
      return;
    }
    const file = form.elements.file.files[0];
    if (!file) {
      showToast('Choose the clean final image first.');
      return;
    }
    const finalPath = await uploadCommissionFinalFile(file);
    const { error } = await state.supabase.from('commission_requests').update({
    ...
    if (error) throw error;
    await state.supabase.from('orders').update({ status: 'fulfilled' }).eq('commission_request_id', data.request_id).eq('status', 'paid');
    showToast('Clean final uploaded for the customer.');
    const dialog = qs('#requestDialog');
    if (dialog && dialog.open) dialog.close();
  }

  if (type === 'support') {
    await callFunction('/.netlify/functions/create-support-ticket', {
      name: data.name,
      email: data.email,
...
async function openRequestDetail(id) {
  if (state.supabase && state.profile) await loadPrivateData();
...
async function downloadCommissionFinal(requestId) {
  const data = await callFunction('/.netlify/functions/download-original', { requestId });
  await downloadImage(data.url, data.title || 'the-art-department-commission-final');
}
...
async function updateSupportStatus(id, status) {
  const { error } = await state.supabase.from('support_tickets').update({ status }).eq('id', id);
  if (error) throw error;
...
function bindEvents() {
  document.addEventListener('click', async (event) => {
    ...
    const downloadCommission = event.target.closest('[data-download-commission]');
    ...
    try {
      ...
      if (downloadCommission) await downloadCommissionFinal(downloadCommission.dataset.downloadCommission);
      ...
      if (openRequest) await openRequestDetail(openRequest.dataset.openRequest);
