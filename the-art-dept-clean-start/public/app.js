...
async function sendArtistApplication(form) {
  if (!form || !form.hasAttribute('data-netlify')) return false;
  try {
...
function bindEvents() {
  ...
  const applicationForm = qs('#artistApplicationForm');
  if (applicationForm) {
    applicationForm.addEventListener('submit', async (event) => {
      event.preventDefault();
      const name = qs('#applicantName').value.trim() || 'New Artist';
      const category = qs('#applicantCategory').value;
      const price = Number(qs('#applicantPrice').value) || 0;
      const rightsOk = qs('#applicantOriginal').checked;
      const payoutLater = qs('#applicantStripe').checked;
      const status = rightsOk ? (payoutLater ? 'Payout setup later' : 'Portfolio review') : 'Needs rights confirmation';
      const sent = await sendArtistApplication(applicationForm);
      artistApplications = [[name, category, price, status]].concat(artistApplications);
      renderDashboard();
      renderApplications();
      showToast(sent ? 'Application sent to Netlify Forms.' : 'Application added to the page preview.');
    });
  }
