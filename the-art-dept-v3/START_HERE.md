...
- commission detail view with quote, payment status, watermarked draft delivery, and private clean final delivery
...
- protected original listing files with watermarked public previews
- watermarked, compressed portfolio previews without storing clean portfolio originals
...
`supabase/commission-workflow-upgrade.sql`

This adds commission detail fields, quote/payment tracking, draft upload support, private final delivery support, and commission order links. It does not delete existing data.

9. Go to Supabase Project Settings > API.
10. Copy:
...
## Commission Safety Workflow

Commission requests start as a budget/brief, not as a guaranteed payment. The safer flow is:

1. Customer sends a request.
2. Artist or admin opens the request details and accepts/quotes it.
3. Customer pays the quote through Stripe.
4. Artist uploads watermarked drafts only after the request shows `paid`.
5. Artist uploads the clean final file in Final delivery.
6. Customer downloads the clean final through a short-lived private link.

This keeps the artist from doing unpaid work and keeps the customer payment inside The Art Department instead of relying on off-site promises.
...
For new portfolio uploads, the browser creates a smaller watermarked preview and uploads only that public preview. The clean portfolio original is not stored on the site, which keeps storage use lower. Existing portfolio pieces stay as-is unless the artist deletes and reuploads them.
