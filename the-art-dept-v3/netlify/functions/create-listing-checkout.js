...
exports.handler = async (event) => {
  ...
  try {
    ...
    const processingFee = processingFeeCents(listing.price_cents, fee);
    const customerTotal = listing.price_cents + fee + processingFee;
    const platformFeeLabel = String(Number(process.env.PLATFORM_FEE_PERCENT || 5)) + '% marketplace fee that helps operate The Art Dept.';
    ...
    if (fee > 0) {
      lineItems.push({
        price_data: {
          ...
          unit_amount: fee,
          product_data: {
            name: 'The Art Dept platform service fee',
    ...
    const session = await stripeClient.checkout.sessions.create({
      ...
      payment_intent_data: {
        ...
        metadata: {
          ...
          listing_id: listing.id,
          artist_id: listing.artist_id,
          artist_amount_cents: String(listing.price_cents),
