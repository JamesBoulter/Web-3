...
function renderArtistCard(item) {
  ...
  return `
    <article class="artist-card">
      ...
      <div class="card-body">
        <div class="card-title"><div><h3>${escapeHtml(item.profile.display_name)}</h3><span>@${escapeHtml(item.artist.handle || 'artist')} - ${escapeHtml(ratingText(item.profile.id))}</span></div><span class="status-pill">${item.listings.length} listings</span></div>
  ...
  return `
    <section class="section-band">
      ...
      <div class="policy-grid">
        <article class="policy-card"><h3>Terms of Service</h3><p>Users must own or have permission to upload the artwork they post. The Art Dept may remove listings, accounts, or content that violates marketplace rules or payment requirements.</p></article>
        <article class="policy-card"><h3>Privacy Policy</h3><p>Account, order, listing, and support information is stored in Supabase. Payment details are handled by Stripe, not stored directly by The Art Dept.</p></article>
...
function renderCustomerPanel(tab) {
  if (tab === 'orders') return renderOrdersTable(state.orders.filter((order) => order.customer_id === state.profile.id), 'Purchase library');
  ...
  const paid = state.orders.filter((order) => order.customer_id === state.profile.id && (order.status === 'paid' || order.status === 'fulfilled'));
  return `
    ...
    <section class="section-band">
      ...
      <form class="form-stack" data-form="listing">
        ...
        <div class="form-grid">
          ...
          <label>Image URL or upload<input name="image_url" value="${editing ? escapeHtml(editing.image_url) : ''}" placeholder="https://..." /></label>
...
function renderManageListingRow(listing) {
  return '<tr><td>' + escapeHtml(listing.title) + '<br><span class="muted">' + escapeHtml(listing.category) + ' - ' + escapeHtml(listing.listing_type) + '</span></td><td>' + formatMoney(listing.price_cents) + '</td><td><span class="status-pill">' + escapeHtml(listing.status) + '</span></td><td><div class="row-actions">' +
...
function renderOrderRow(order) {
  ...
  const customerActions = isCustomerOrder && (order.status === 'paid' || order.status === 'fulfilled')
    ? review
      ...
      : '<button class="secondary compact" data-review-order="' + order.id + '">Review</button>'
    : '';
...
function renderRequestsTable(requests, title, context = '') {
  return `
    <section class="table-wrap">
      <h2>${escapeHtml(title)}</h2>
      ${requests.length ? '<table><thead><tr><th>Project</th><th>Customer</th><th>Artist</th><th>Budget</th><th>Status</th><th>Action</th></tr></thead><tbody>' +
...
function renderRequestRow(request, context) {
  ...
  const adminAction = context === 'admin'
    ? '<div class="row-actions"><select class="compact-select" data-assign-request="' + request.id + '"><option value="">Match needed</option>' + artistOptions + '</select><button class="secondary compact" data-request-status="' + request.id + '" data-status="completed">Complete</button><button class="danger compact" data-request-status="' + request.id + '" data-status="declined">Close</button></div>'
    : '';
  const artistAction = context === 'artist' && request.artist_id === state.profile.id
    ? '<div class="row-actions"><button class="secondary compact" data-request-status="' + request.id + '" data-status="accepted">Accept</button><button class="danger compact" data-decline-request="' + request.id + '">Decline</button><button class="secondary compact" data-request-status="' + request.id + '" data-status="completed">Complete</button></div>'
    : '';
  ...
  return '<tr><td>' + escapeHtml(request.title) + '<br><span class="muted">' + escapeHtml(request.brief).slice(0, 160) + '</span>' + (request.declined_reason ? '<br><span class="muted">Declined reason: ' + escapeHtml(request.declined_reason) + '</span>' : '') + '</td><td>' + escapeHtml(request.name) + '<br><span class="muted">' + escapeHtml(request.email) + '</span></td><td>' + escapeHtml(request.artist_id ? artistName(request.artist_id) : 'Match needed') + '</td><td>' + formatMoney(request.budget_cents) + '</td><td><span class="status-pill">' + escapeHtml(request.status) + '</span></td><td>' + (adminAction || artistAction || customerAction || '<span class="muted">No action</span>') + '</td></tr>';
}
...
async function reviewOrder(id) {
  ...
  if (!order) return;
  const ratingValue = prompt('Rating from 1 to 5 stars?');
  ...
  const body = prompt('Optional written review:') || '';
...
function bindEvents() {
  document.addEventListener('click', async (event) => {
    ...
    try {
      ...
      if (reviewButton) await reviewOrder(reviewButton.dataset.reviewOrder);
