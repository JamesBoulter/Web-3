...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!orderId && !requestId) return json(400, { error: 'Order ID or commission request ID is required.' });

    const supabase = supabaseAdmin();
    const { data: profile } = await supabase.from('profiles').select('id, role').eq('id', user.id).single();

    if (requestId) {
      const [{ data: request, error: requestError }, { data: order }] = await Promise.all([
        supabase
          .from('commission_requests')
          .select('id, customer_id, artist_id, title, final_file_path, payment_status, status')
          .eq('id', requestId)
          .single(),
        supabase
          .from('orders')
          .select('id, customer_id, artist_id, commission_request_id, status')
      ...
      const isBuyer = order && order.customer_id === user.id && ['paid', 'fulfilled'].includes(order.status);
      const isArtist = request.artist_id === user.id;
      const isAdmin = profile && profile.role === 'admin';
      if (!isBuyer && !isArtist && !isAdmin) return json(403, { error: 'This final file is not available for your account.' });

      if (!request.final_file_path) return json(404, { error: 'The final file has not been uploaded yet.' });

      const { data, error } = await supabase
        .storage
        .from('listing-originals')
        .createSignedUrl(request.final_file_path, 120);

      if (error || !data || !data.signedUrl) return json(500, { error: 'The final file could not be prepared.' });

      return json(200, {
        url: data.signedUrl,
        title: request.title + ' final',
        protected: true,
