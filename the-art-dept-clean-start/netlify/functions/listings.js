...
const { getListingStore, listingStoreErrorMessage } = require('./_listing-store');
...
function listingFromBody(body) {
  ...
  return {
    ...
    connectedAccountId: requireText(body.connectedAccountId, 'Connected Stripe artist account', 80),
...
function publicListing(listing) {
  return {
    ...
    image: listing.image,
    canBuy: !!listing.connectedAccountId,
    delivery: listing.delivery
  };
}

async function listListings() {
  const store = await getListingStore();
...
exports.handler = async (event) => {
  try {
    ...
    return json(error.statusCode || 500, {
      error: error.statusCode ? error.message : listingStoreErrorMessage(error)
    });
  }
};
